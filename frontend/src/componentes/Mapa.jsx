...
// Faixas calibradas ao problema real (taxa base ~1%/dia no RN).
function corPorProba(p) {
  if (p == null || isNaN(p)) return "#d6d3d1";
  if (p < 0.005) return "#15803d";
  if (p < 0.01)  return "#84cc16";
  if (p < 0.02)  return "#eab308";
  if (p < 0.04)  return "#ea580c";
  return "#b91c1c";
}
...
export default function Mapa({ geojson, mapaProb, dadosComparacao, topNCods, onClicarMun }) {
  if (!geojson) {
    return <div className="spinner">carregando mapa...</div>;
  }
  ...
  const estilo = (feat) => {
    ...
    if (dadosComparacao) {
      ...
      if (d){
         cor = corComparacaoTopN(topNCods && topNCods.has(cod), d.teve_foco);
      }
    } 
    else if (mapaProb) {
